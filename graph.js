...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Graph{
        constructor(){
            this.xaxis = new Line(0, canvas.height*.5, canvas.width, canvas.height*.5, "white", 2)
            this.yaxis = new Line(canvas.width*.5, 0, canvas.width*.5, canvas.height, "white", 2)
            this.dot = new Circle(0,0, 3, "red")
            this.yaxis.draw()
            this.xaxis.draw()
        }
        draw(){
            this.dot.x++
            this.dot.y = (canvas.height*.5)-  ((this.dot.x-(canvas.width*.5))-(((this.dot.x-(canvas.width*.5))/1000)*((this.dot.x-(canvas.width*.5)))))
            this.dot.draw()
        }
